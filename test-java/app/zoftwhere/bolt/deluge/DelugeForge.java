...
class DelugeForge {
  ...
  private final RunStandardArgued runStandardArgued =
      (arguments, scanner, out) -> {
  ...
  private final RunStandard runStandard =
      (scanner, out) -> runStandardArgued.call(null, scanner, out);

  private final RunConsoleArgued runConsoleArgued =
      (arguments, inputStream, outputStream) -> {
        if (globalError.get() != null) {
          throw globalError.get();
        }
  ...
  private final RunConsole runConsole =
      (inputStream, outputStream) -> runConsoleArgued.call(null, inputStream, outputStream);

  DelugeForge(
      List<Charset> encodingList, List<DelugeSetting> settingList, List<DelugeData> inputList) {
  ...
  private RunnerProgram forProgram(
      RunnerInterface runner, DelugeProgramType type, DelugeSetting setting) {
    if (PROGRAM_STANDARD == type) {
      ...
      if (setting.hasCharSet()) {
        return runner.run(setting.charset(), runStandardArgued).argument(setting.argumentArray());
      }
      return runner.run(runStandardArgued).argument(setting.argumentArray());
      ...
      return runner.runConsole(runConsoleArgued).argument(setting.argumentArray());
    } else {
  ...
  private RunnerProgramOutput withProgram(
      RunnerProgramInput programInput, DelugeProgramType type, DelugeSetting setting) {
    if (INPUT_STANDARD == type) {
      ...
      return programInput.argument(setting.argumentArray()).run(runStandardArgued);
      ...
      return programInput.argument(setting.argumentArray()).runConsole(runConsoleArgued);
    } else {
      final var cause = new Exception("type: " + type);
      throw new DelugeException("deluge.program.type.switch.default", cause);
    }
